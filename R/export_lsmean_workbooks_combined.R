...
#'
#' @title
#' @param combined_lsmeans
#' @param export_directory
export_lsmean_workbooks_combined <- function(phenotype_lsmeans = combined_lsmeans, export_directory
                                             = here("exports",
                                             "combined_lsmeans")) {
  ...
  lsmeans_final_report <- function(test, data){
    ...
    remove_loc <- function(colname){
      ...
      ifelse(length(split_name) == 1, split_name[[1]], paste(split_name[[2]], split_name[[3]], sep = " - "))
